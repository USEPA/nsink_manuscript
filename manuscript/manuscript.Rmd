...
niantic_static_maps <- nsink_generate_static_maps(niantic_data, niantic_removal, 900)
```
...
Comparing the N delivery from two locations that are separated by approximately 900 meters illustrates the importance of location with respect to landscape N sinks. Approximately 56% of the N that originates at point A is transported to the estuary vs. approximately 25% of the N that originates at point B (Fig 4). The difference is due to the different N sinks encountered along the way to the outlet or estuary. Most of the removal that occurs along flowpath A is due to flow through hydric (wetland) soils, with little removal occurring during flow along the downgradient stream network (Table 2). Flowpath B encounters two ponds along the way, significantly increasing residence time and N transformation (Table 3).

```{r plots4, echo = FALSE, message = FALSE, warning = FALSE, fig.cap = "Figure 4: Flowpaths originating from two nearby locations with differing N removal estimates. Hydrofeatures are pictured on the NTI (Nitrongen Transport Index) map."}
...
The maps produced by nsink can be helpful in prioritizing source controls, sink protection and/or restoration. While wetlands are considered "protected" by law they are frequently altered through variances granted by local commissions or committees charged with zoning decisions. By better understanding the role sinks play in the protection of estuaries, local agencies have a better chance of achieving protection of downstream receiving water. Variances granted on a case=by-case basis can, however unintentionally, quickly undermine local water quality goals. 

Similarly, sources of N can be highlighted using the nsink static maps [Fig XX]. The N Delivery Index map combines N sources with N transport to highlight 


Potential ways to use N-Sink are also available as [bonus maps](https://uconnclear.maps.arcgis.com/apps/MapSeries/index.html?appid=e96b01502f5b4aeda64aacf0cb5234c7&folderid=dde199a7cf674845a83b43a3011b1d31) on the N-Sink website.
