...
The `nsink` package provides several functions (Table 1) to set up and run an N-Sink analysis for a specified 12-digit HUC. By using the package all required data are downloaded, prepared for the analysis, HUC-wide N removal calculated, and flowpaths summarized.  Additionally, a convenience function, `nsink_build()`, is included that will run all of the required functions for a specified HUC. The workflow follows a simple series of steps to create a set of static maps: N Removal Efficiency, N Transport Efficiency, and N Delivery Index.
...
knitr::kable(table1, caption = "Table 1: Functions in the R package `nsink`")
```
...
knitr::kable(niantic_fp_removal_a, caption = "Table 2: Summary of N removal along specified flowpath A (See Figure 4)")
```
## Static maps
...
```{r plot1, fig.cap = "Figure 1: Nitrogen Removal Efficiency (range 0 to 1) for the Niantic River HUC-12, showing the estimated percentage of N removed within three types of landscape N sinks - wetlands, streams, lakes/ponds/reservoirs."}

nsink_plot(niantic_static_maps, map = "removal")
```
```{r plot2, fig.cap = "Figure 2: Nitrogen Transport Index (range 0 to 100%) for the Niantic River HUC-12, showing the estimated percentage of N originating at a given location within a watershed that is expected to reach downstream receiving waters, such as a coastal embayment."}
...
```{r plot4, echo = FALSE, message = FALSE, warning = FALSE, fig.cap = "Figure 4: Flowpaths originating from two nearby locations within the Niantic River HUC-12 with differing N removal estimates. Hydrofeatures are pictured on the Nitrogen Transport Index map."}
...
plot(streams, add = TRUE, col = "dodgerblue")
...
```{r plot5a, include = TRUE, echo = FALSE, warning = FALSE, fig.cap = "Figure 5A: Land cover within an area of interest in the Niantic River HUC-12, highlighting two developed areas."}
...
```
```{r, plot5b, include = TRUE, echo = FALSE, warning = FALSE, fig.cap = "Figure 5B: Nitrogen Delivery Index over land cover, with hotter colors denoting higher N delivery based on normalized loading from different land covers multiplied by the N Transport Efficiency."}
