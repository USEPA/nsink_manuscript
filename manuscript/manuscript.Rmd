...
author:
- "Dorothy Q Kellogg^*1*^^\\*^, Jeffrey W. Hollister^*2*^, Chester L. Arnold^*3*^, Arthur J. Gold^*1*^, Emily H. Wilson^*3*^, Cary B. Chadwick^*3*^, David W. Dickson^*3*^, Qian Lei-Parent^*3*^, Kenneth J. Forshay^*4*^"
- ^*1*^ Department of Natural Resources Science, University of Rhode Island, Kingston, RI 02881, USA
- ^*2*^ Office Of Research and Development, Atlantic Coastal Environmental Sciences Division, US Environmental Protection Agency, Narragansett, RI 02882, USA
- ^*3*^ Center for Land Use Education and Research, University of Connecticut, Haddam, CT 06438, USA
- ^*4*^ Office Of Research and Development, Groundwater Characterization and Remediation Division, US Environmental Protection Agency, Ada, OK 74820, USA
- '^\*^Corresponding Author: 
Dorothy Q Kellogg
Department of Natural Resources Science
University of Rhode Island, Kingston, RI 02881, USA
Email address: <qkellogg@uri.edu>'
bibliography: manuscript.bib
output:
  word_document:
    keep_md: true
    reference_docx: manuscript_f1000_template.docx
  html_document:
    df_print: paged
...
```{r plot1, fig.cap = "Figure 1: Nitrogen Removal Efficiency (range 0 to 1) for the Niantic River HUC-12, showing the estimated fraction of N removed within three types of landscape N sinks - wetlands, streams, lakes/ponds/reservoirs.", echo=FALSE}
...
Operation
============
The R package *nsink* requires R version >= 3.5.0. The *nsink* package is available from https://github.com/usepa/nsink and is fully described in @hollister2022nsink.

Use Cases
============

## Use Case 1: Comparison of N delivery from two adjacent locations
...
```{r plot4, eval = TRUE, echo = FALSE, message = FALSE, warning = FALSE, fig.cap = "Figure 4: Flowpaths originating from two nearby locations within the Niantic River HUC-12 with differing N removal estimates. Hydrofeatures are pictured on the Nitrogen Transport Index map.", fig.height = 10, fig.width = 7.5}
...
tnsport_idx <- classify(niantic_static_maps$transport_idx,matrix(c(from,to,to),ncol = 3))
...
## Use Case 2: Source controls and sink protection or restoration
...
```{r plot5a, eval = TRUE, include = TRUE, echo = FALSE, warning = FALSE, fig.cap = "Figure 5A: Land cover within an area of interest in the Niantic River HUC-12, highlighting two similarly developed areas (summary given in Table 4)."}
...
leg <- legend_nlcd[legend_nlcd$Class %in% vals$Class,]
...
# extract/mask LULC values from west and east
lu_w <- terra::mask(niantic_data$nlcd, vect(ptw_buf)) # don't use case2 as data lost when cropped
lu_e <- terra::mask(niantic_data$nlcd, vect(pte_buf))

luw_sum <- terra::freq(lu_w) %>% dplyr::select(value, count)
names(luw_sum) <- c("value", "count_w")
lue_sum <- terra::freq(lu_e) %>% dplyr::select(value, count)
names(lue_sum) <- c("value", "count_e")

lu_sum <- full_join(luw_sum, lue_sum)

# aggregate
dev <- c("Developed, Open Space", "Developed, Low Intensity", 
         "Developed, Medium Intensity","Developed, High Intensity")
ag <- c("Pasture/Hay", "Cultivated Crops")
forest_veg <- c("Deciduous Forest", "Evergreen Forest", "Mixed Forest",
                "Dwarf Scrub", "Shrub/Scrub", "Grassland/Herbaceous",
                "Sedge/Herbaceous", "Lichens", "Moss")
wetland <- c("Woody Wetlands", "Emergent Herbaceous Wetlands")
water <- "Open Water"
other <- c("Perennial Ice/Snow", "Barren Land (Rock/Sand/Clay)")

lu_sum$agg <- "Developed"
lu_sum$agg[lu_sum$value %in% ag] <- "Agriculture"
lu_sum$agg[lu_sum$value %in% forest_veg] <- "Forest/Vegetated"
lu_sum$agg[lu_sum$value %in% wetland] <- "Wetlands"
lu_sum$agg[lu_sum$value == "Open Water"] <- "Open Water"
lu_sum$agg[lu_sum$value %in% other] <- "Other"

lu_agg <- lu_sum %>% group_by(agg) %>% 
  summarize(sum_w = sum(count_w, na.rm = TRUE), 
            sum_e = sum(count_e, na.rm = TRUE)) %>%
  mutate(percent_w = round(100*sum_w/sum(sum_w, na.rm = TRUE)),
         percent_e = round(100*sum_e/sum(sum_e, na.rm = TRUE)))

dev_pos <- which(lu_agg$agg == "Developed")
ag_pos <- which(lu_agg$agg == "Agriculture")
for_pos <- which(lu_agg$agg == "Forest/Vegetated")
wet_pos <- which(lu_agg$agg == "Wetlands")
other_pos <- which(lu_agg$agg == "Other")

table4 <- data.frame("Land Cover" = c("Developed", "Agriculture", "Forest/Vegetated",
                                      "Wetlands", "Other"),
                     "West (%)" = c(lu_agg$percent_w[dev_pos], lu_agg$percent_w[ag_pos],
                                    lu_agg$percent_w[for_pos], lu_agg$percent_w[wet_pos],
                                    lu_agg$percent_w[other_pos]),
                     "East (%)" = c(lu_agg$percent_e[dev_pos], lu_agg$percent_e[ag_pos],
                                    lu_agg$percent_e[for_pos], lu_agg$percent_e[wet_pos],
                                    lu_agg$percent_e[other_pos]),
                     check.names = FALSE)
```


```{r table4, include = TRUE, echo = FALSE}
knitr::kable(table4, caption = "Table 4: Summary of land cover in highlighted areas of interest.")
```


```{r, plot5b, eval = TRUE, include = TRUE, echo = FALSE, warning = FALSE, fig.cap = "Figure 5B: Nitrogen Transport Index within an area of interest in the Niantic River HUC-12, highlighting two similarly developed areas."}
...
from <- c(seq(0,90,10), NA)
to <- c(seq(10,100,10), NA)
delivery_idx <- classify(niantic_static_maps$delivery_idx,matrix(c(from,to,to),ncol = 3))
...
Data availability
=================
Datasets used by nsink include the National Hydrography Dataset Plus (NHDPlus), Soil Survey Geographic Database (SSURGO), the National Land Cover Dataset (NLCD) land cover and the National Landcover Dataset (NLCD) Impervious Surface [@jin2019overall; @moore2019user; @soil2017web]. All are available via the FedData package in R [@feddata].
...
Archived source code at the time of publication: DOI: 10.5281/zenodo.10045141
...
Grant information
=================
The research was supported, in part, by a cooperative agreement with the U.S. Environmental Protection Agency's Office of Research and Development, EPA Assistant Agreement Number CR83680001, "N-Sink tool and outreach to two watersheds in New England."
